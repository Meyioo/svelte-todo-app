<script lang="ts">
	...
	import PriorityFilter from './priority-filter.svelte';
	import Searchbar from './searchbar.svelte';

	const { title, showSearch = false, showPriorityFilter = false }: IHeaderProps = $props();
</script>
...
<header class="sticky top-0 w-full border-b border-gray-200 bg-gray-50">
	<div class="mx-auto max-w-screen-xl px-4 py-8 sm:px-6 sm:py-12 lg:px-8">
		<div class="flex justify-between gap-4 md:flex-row md:items-center md:justify-between">
			<h1 class="my-auto text-2xl font-bold text-gray-900 sm:text-3xl">{title}</h1>
